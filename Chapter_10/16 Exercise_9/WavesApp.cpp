...
{
	md3dImmediateContext->ClearRenderTargetView(CurrentBackBufferView().Get(), Colors::Black);
	...
	/// 绘制不透明物体.
	/// </summary>
	md3dImmediateContext->OMSetDepthStencilState(mDepthStencilStates["blend"].Get(), 0);
...
void WavesApp::BuildRasterizerStates()
{
	...
	/// 渲染深度模板状态.
	///
	D3D11_DEPTH_STENCIL_DESC rsDDS;
	rsDDS.DepthEnable = TRUE;
	rsDDS.DepthFunc = D3D11_COMPARISON_ALWAYS;
	rsDDS.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;
	rsDDS.StencilEnable = TRUE;
	rsDDS.StencilReadMask = 0xff;
	rsDDS.StencilWriteMask = 0xff;

	rsDDS.FrontFace.StencilFunc = D3D11_COMPARISON_ALWAYS;
	rsDDS.FrontFace.StencilPassOp = D3D11_STENCIL_OP_KEEP;
	rsDDS.FrontFace.StencilFailOp = D3D11_STENCIL_OP_KEEP;
	rsDDS.FrontFace.StencilDepthFailOp = D3D11_STENCIL_OP_KEEP;

	rsDDS.BackFace.StencilFunc = D3D11_COMPARISON_ALWAYS;
	rsDDS.BackFace.StencilPassOp = D3D11_STENCIL_OP_KEEP;
	rsDDS.BackFace.StencilFailOp = D3D11_STENCIL_OP_KEEP;
	rsDDS.BackFace.StencilDepthFailOp = D3D11_STENCIL_OP_KEEP;

	HR(md3dDevice->CreateDepthStencilState(&rsDDS, mDepthStencilStates["blend"].GetAddressOf()));
}
