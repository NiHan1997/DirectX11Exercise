...
void WavesApp::DrawScene()
{
	...
	// 标记模板值.
	md3dImmediateContext->OMSetDepthStencilState(mDepthStencilStates["markColor"].Get(), 0);
	...
	// 模板值标记完成, 开始渲染屏幕后处理效果.
	md3dImmediateContext->VSSetShader(mVertexShaders["screenPostVS"].Get(), nullptr, 0);
	md3dImmediateContext->PSSetShader(mPixelShaders["screenPostPS"].Get(), nullptr, 0);

	for (int i = 1; i <= 5; ++i)
	{
		md3dImmediateContext->OMSetDepthStencilState(mDepthStencilStates["color"].Get(), i);

		StencilConstants stencilConstants;
		stencilConstants.StencilValue = i;
		d3dUtil::CopyDataToGpu(md3dImmediateContext.Get(), &stencilConstants,
			sizeof(StencilConstants), mConstantBuffers["stencil"].Get());

		md3dImmediateContext->PSSetConstantBuffers(0, 1, mConstantBuffers["stencil"].GetAddressOf());

		DrawFullScreenQuad(md3dImmediateContext.Get());
	}
...
void WavesApp::BuildConstantBuffers()
{
	...
	/// 模板参考值的常量数据.
	D3D11_BUFFER_DESC stencilCbd;
	stencilCbd.Usage = D3D11_USAGE_DYNAMIC;
	stencilCbd.ByteWidth = sizeof(StencilConstants);
	stencilCbd.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	stencilCbd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	stencilCbd.MiscFlags = 0;
	stencilCbd.StructureByteStride = 0;

	HR(md3dDevice->CreateBuffer(&stencilCbd, nullptr, mConstantBuffers["stencil"].GetAddressOf()));
}
...
void WavesApp::BuildShadersAndInputLayout()
{
	...
	ComPtr<ID3DBlob> screenPostVertexBlob = d3dUtil::CompileShader(L"Shaders/ScreenPost.hlsl", nullptr, "VS", "vs_5_0");
	ComPtr<ID3DBlob> screenPostPixelBlob = d3dUtil::CompileShader(L"Shaders/ScreenPost.hlsl", nullptr, "PS", "ps_5_0");

	HR(md3dDevice->CreateVertexShader(screenPostVertexBlob->GetBufferPointer(), screenPostVertexBlob->GetBufferSize(),
		nullptr, mVertexShaders["screenPostVS"].GetAddressOf()));
	HR(md3dDevice->CreatePixelShader(screenPostPixelBlob->GetBufferPointer(), screenPostPixelBlob->GetBufferSize(),
		nullptr, mPixelShaders["screenPostPS"].GetAddressOf()));
...
void WavesApp::BuildRasterizerStates()
{
	...
	/// 颜色模板值标记的渲染.
	///
	D3D11_DEPTH_STENCIL_DESC markColorDDS;
	markColorDDS.DepthEnable = TRUE;
	markColorDDS.DepthFunc = D3D11_COMPARISON_LESS;
	markColorDDS.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;
	markColorDDS.StencilEnable = TRUE;
	markColorDDS.StencilReadMask = 0xff;
	markColorDDS.StencilWriteMask = 0xff;

	markColorDDS.FrontFace.StencilFunc = D3D11_COMPARISON_ALWAYS;
	markColorDDS.FrontFace.StencilPassOp = D3D11_STENCIL_OP_INCR;
	markColorDDS.FrontFace.StencilFailOp = D3D11_STENCIL_OP_KEEP;
	markColorDDS.FrontFace.StencilDepthFailOp = D3D11_STENCIL_OP_KEEP;

	markColorDDS.BackFace.StencilFunc = D3D11_COMPARISON_ALWAYS;
	markColorDDS.BackFace.StencilPassOp = D3D11_STENCIL_OP_INCR;
	markColorDDS.BackFace.StencilFailOp = D3D11_STENCIL_OP_KEEP;
	markColorDDS.BackFace.StencilDepthFailOp = D3D11_STENCIL_OP_KEEP;

	HR(md3dDevice->CreateDepthStencilState(&markColorDDS, mDepthStencilStates["markColor"].GetAddressOf()));

	///
	/// 读取模板值, 根据不同模板值决定颜色.
	///
	D3D11_DEPTH_STENCIL_DESC colorDDS;
	colorDDS.DepthEnable = TRUE;
	colorDDS.DepthFunc = D3D11_COMPARISON_ALWAYS;
	colorDDS.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO;
	colorDDS.StencilEnable = TRUE;
	colorDDS.StencilReadMask = 0xff;
	colorDDS.StencilWriteMask = 0xff;

	colorDDS.FrontFace.StencilFunc = D3D11_COMPARISON_EQUAL;
	colorDDS.FrontFace.StencilPassOp = D3D11_STENCIL_OP_KEEP;
	colorDDS.FrontFace.StencilFailOp = D3D11_STENCIL_OP_KEEP;
	colorDDS.FrontFace.StencilDepthFailOp = D3D11_STENCIL_OP_KEEP;

	colorDDS.BackFace.StencilFunc = D3D11_COMPARISON_EQUAL;
	colorDDS.BackFace.StencilPassOp = D3D11_STENCIL_OP_KEEP;
	colorDDS.BackFace.StencilFailOp = D3D11_STENCIL_OP_KEEP;
	colorDDS.BackFace.StencilDepthFailOp = D3D11_STENCIL_OP_KEEP;

	HR(md3dDevice->CreateDepthStencilState(&colorDDS, mDepthStencilStates["color"].GetAddressOf()));
}
...
void WavesApp::DrawFullScreenQuad(ID3D11DeviceContext* context)
{
