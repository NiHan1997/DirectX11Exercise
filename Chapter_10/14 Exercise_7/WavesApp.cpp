...
{
	std::vector<std::wstring> texturePath(62);
	texturePath[0] = L"../../Textures/grass.dds";
	texturePath[1] = L"../../Textures/water1.dds";

	for (int i = 0; i < 60; ++i)
	{
		if (i < 9)
			texturePath[2 + i] = L"Textures/Bolt00" + std::to_wstring(i + 1) + L".DDS";
		else
			texturePath[2 + i] = L"Textures/Bolt0" + std::to_wstring(i + 1) + L".DDS";
	}
...
void WavesApp::BuildCylinderBuffers()
{
	GeometryGenerator geoGen;
	GeometryGenerator::MeshData cylinder;
	geoGen.CreateCylinder(5.0f, 5.0f, 15.0f, 20, 20, cylinder);
	...
	std::vector<UINT> indices = cylinder.Indices;
	...
	HR(md3dDevice->CreateBuffer(&vbd, &vInitData, mVertexBuffers["cylinder"].GetAddressOf()));
	...
	HR(md3dDevice->CreateBuffer(&ibd, &iInitData, mIndexBuffers["cylinder"].GetAddressOf()));
	...
	geo->VertexBuffer = mVertexBuffers["cylinder"].Get();
	geo->IndexBuffer = mIndexBuffers["cylinder"].Get();
...
void WavesApp::BuildMaterials()
{
	...
	fence->Name = "lightning";
	fence->MatCBIndex = 2;
	fence->DiffuseSrvHeapIndex = 2;
	fence->DiffuseAlbedo = { 1.0f, 1.0f, 1.0f, 0.5f };
	fence->FresnelR0 = { 0.1f, 0.1f, 0.1f };
	fence->Roughness = 0.25f;

	mMaterials["grass"] = std::move(grass);
	mMaterials["water"] = std::move(water);
	mMaterials["lightning"] = std::move(fence);
}
...
void WavesApp::BuildRenderItems()
{
	...
	cylinderRitem->ObjectCBIndex = 2;
	cylinderRitem->Geo = mGeos["cylinderGeo"].get();
	cylinderRitem->Mat = mMaterials["lightning"].get();
	XMStoreFloat4x4(&cylinderRitem->TexTransform, XMMatrixScaling(3.0f, 3.0f, 1.0f));
	...
	cylinderRitem->VertexBuffer = cylinderRitem->Geo->VertexBuffer.Get();
	cylinderRitem->IndexBuffer = cylinderRitem->Geo->IndexBuffer.Get();
	mRitemLayer[(int)RenderLayer::Transparent].push_back(cylinderRitem.get());
...
void WavesApp::DrawRenderItems(ID3D11DeviceContext* context, const std::vector<RenderItem*>& ritems)
{
	for (size_t i = 0; i < ritems.size(); i++)
	{
		...
		// 纹理贴图数据.
		if (ri->Mat->Name == "lightning")
		{
			static UINT lightningIndex = 0;
			lightningIndex = lightningIndex % 59 + 2;
			context->PSSetShaderResources(0, 1, mTextureSrvs[lightningIndex].GetAddressOf());
		}
		else
		{
