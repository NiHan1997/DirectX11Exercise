...
void WavesApp::DrawScene()
{
	...
	md3dImmediateContext->GSSetShader(mGeometryShaders["normalGS"].Get(), nullptr, 0);
...
void WavesApp::BuildShadersAndInputLayout()
{
	...
	{
		"FOG", "1",
		//"ALPHA_TEST", "1",  // 透明度测试逻辑会影响箱子网格法线显示.
		NULL, NULL
	};
	...
	ComPtr<ID3DBlob> normalGeometryBlob = d3dUtil::CompileShader(L"Shaders/Default.hlsl", nullptr, "GS", "gs_5_0");	
	...
	HR(md3dDevice->CreateGeometryShader(normalGeometryBlob->GetBufferPointer(), normalGeometryBlob->GetBufferSize(),
		nullptr, mGeometryShaders["normalGS"].GetAddressOf()));
...
void WavesApp::BuildRasterizerStates()
{
	...
	alphaTestRSDesc.FillMode = D3D11_FILL_WIREFRAME;
